...
def camStack(config_path_list, stack_time = datetime.datetime.utcnow() - datetime.timedelta(seconds = 30), border=5, background_compensation=True,
    ...
    if out_dir is not None:
        filenam = os.path.join(os.path.expanduser(out_dir), os.path.basename(cml_args.time) + "_cam_stack.jpg")
    else:
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('-t', '--time', type=str,
                            help="""Time of plot e.g. 20231231_010203""")
    ...
    plot_date = cml_args.time.split('_')[0]
    plot_time = cml_args.time.split('_')[1]
    ...
    stack_time = datetime.datetime(pyr,pmth,pday,phr,pmin,psec)
