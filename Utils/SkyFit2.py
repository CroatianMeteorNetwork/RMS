...
from RMS.Astrometry.Conversions import date2JD, JD2HourAngle, trueRaDec2ApparentAltAz, \
    apparentAltAz2TrueRADec, altAz2RADec, raDec2AltAz, J2000_JD, jd2Date, datetime2JD, JD2LST, geo2Cartesian, vector2RaDec, raDec2Vector
...
class PlateTool(QtWidgets.QMainWindow):
    def __init__(self, input_path=None, config=None, beginning_time=None, fps=None, gamma=None,
        ...
        self.override_max_global_intensity = 230
    ...
    def setupUI(self, loaded_file=False):
        ...
        self.tab.star_detection.sigMaxGlobalIntensityChanged.connect(self.updateMaxGlobalIntensity)
    ...
    def updateMaxStars(self, value):
        """ Update the SkyFit session max stars parameter (used for re-detection, never saved). """
        self.override_max_stars = value

    def updateConfigMaxStars(self, value):
        """ Update the max_stars value that Save Config writes to the station config. """
        self.override_config_max_stars = value
        self._updateConfigSaveButtonState()

    def updateMaxGlobalIntensity(self, value):
        """ Update max global intensity (too-bright image cutoff) override parameter. """
        self.override_max_global_intensity = value
        self._updateConfigSaveButtonState()
    ...
    def _countTrueFalsePositives(self, ff_name, gate_factor, segment_radius,
        ...
        Arguments:
            ...
            scale_radius_with_fwhm: [bool] Widen the match radius to 2.1x the fitted FWHM
    ...
    def _extractStarsCurrentImage(self, ff_name, extra_info=None):
        ...
        if self.img_handle.input_type == 'ff':
            ...
            max_global_intensity = getattr(self.config, 'max_global_intensity', 230)*(2**(bit_depth - 8))
    ...
    def filterBlendedStars(self, fwhm_mult=DEFAULT_BLEND_FWHM_MULT, mag_margin=0.3, pull_px=None):
        """
    ...
    def tryQuickAlignment(self, pointing_only=False):
        ...
        if len(self.paired_stars) >= 15:
            removed = self.filterBlendedStars(fwhm_mult=DEFAULT_BLEND_FWHM_MULT, mag_margin=0.3, pull_px=2.0)
            if removed > 0:
    ...
    def getFOVcentre(self):
        ...
        if self.platepar.refraction:
            alt_true = np.degrees(pyRefractionApparentToTrue(np.radians(self.alt_centre),
                                                           refractionScale(self.platepar.elev)))
    ...
    def findMatchingPairs(self):
        ...
        # Match radius scaled to the star size on this image (multiplier rescaled 2.0 -> 2.8
        # to preserve the pixel radius after the sqrt(2) FWHM convention fix in ExtractStars;
        # the fallback and the clamp below are absolute pixels)
        good_fwhm = det_fwhm[det_fwhm > 0]
        match_radius = 2.8*float(np.median(good_fwhm)) if len(good_fwhm) else 6.0
