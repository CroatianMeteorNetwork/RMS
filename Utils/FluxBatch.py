...
def combineFixedBinsAndComputeFlux(sol_bins, meteors, time_area_prod, min_meteors=50, ci=0.95):
    """
    ...
    for end_idx in range(1, len(meteors)):
        ...
        if np.sum(meteors[sl]) >= min_meteors:
            start_idx = end_idx
    ...
    return np.array(sol_list), np.array(flux_list), np.array(flux_lower_list), np.array(flux_upper_list),\
        np.array(meteor_count_list), np.array(area_time_product_list)


def calculateFixedBins(file_data, bin_duration=5):
    """
    ...
    all_time_intervals = [time_interval for data in file_data for time_interval in data[4]]
    for i in all_time_intervals:
        print(i)
    ...
    sol_bins = np.arange(sol_beg[start_idx],
                            sol_end[end_idx] if sol_beg[start_idx] < sol_end[end_idx] else sol_end[end_idx] + 2*np.pi,
                            2*np.pi/60/24/365.24219*bin_duration)
    # calculating datetime corresponding to sol_bins for each year
    bin_datetime_dict = []
    bin_datetime = []
    for sol in sol_bins:
        curr_time = all_time_intervals[start_idx][0] + datetime.timedelta(minutes=(sol - sol_bins[0])/(2*np.pi)*365.24219*24*60)
    ...
    for start_time, _ in all_time_intervals:
        if all([year_start > start_time or start_time > year_end for (year_start, year_end), _ in bin_datetime_dict]):
            delta_years = int(np.floor((start_time - all_time_intervals[start_idx][0]).total_seconds()/(365.24219*24*60*60)))
            bin_datetime = [jd2Date(solLon2jdSteyaert(dt.year + delta_years, dt.month, sol), dt_obj=True)
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("batch_path", metavar="BATCH_PATH", type=str,
        help="Path to the flux batch file or csv file.")
    arg_parser.add_argument("--output_filename", metavar="FILENAME", type=str, default='fluxbatch_output',
        help="Filename to export images and data (exclude file extensions)")
    arg_parser.add_argument("-csv", action='store_true',
        help="Whether batch_path is a .csv file that will be loaded from")
    ...
    if not fluxbatch_cml_args.csv:
        with open(fluxbatch_cml_args.batch_path) as f:
            ...
            for line in f:
                ...
                ftpdetectinfo_path, shower_code, s, binduration, binmeteors, time_intervals, fwhm, ratio_threshold = \
                        flux_cml_args.ftpdetectinfo_path, flux_cml_args.shower_code, flux_cml_args.s, flux_cml_args.binduration, \
                        flux_cml_args.binmeteors, flux_cml_args.timeinterval, flux_cml_args.fwhm, flux_cml_args.ratiothres
                ...
                file_data.append([config, ftp_dir_path, ftpdetectinfo_path, shower_code, time_intervals, s,
                                  binduration, binmeteors, fwhm])

            sol_bins, bin_datetime_dict = calculateFixedBins(file_data)

            all_bin_information = []
            # Compute the flux
            for config, ftp_dir_path, ftpdetectinfo_path, shower_code, time_intervals, s, \
                                  binduration, binmeteors, fwhm in file_data:
                # print(config.stationID, )
                for interval in time_intervals:
                    print(f'Using interval: {interval}')
                    dt_beg, dt_end = interval
                    forced_bins = (bin_datetime_dict[np.argmax(
                        [year_start < dt_beg < year_end for (year_start, year_end), _ in bin_datetime_dict])][1],
                                   sol_bins)
                    ret = computeFlux(config, ftp_dir_path, ftpdetectinfo_path, 
                        shower_code, dt_beg, dt_end, s, binduration, binmeteors, show_plots=False,
                        default_fwhm=fwhm, forced_bins=forced_bins, confidence_interval=ci)
                    if ret is None:
                        continue
                    sol_data, flux_lm_6_5_data, flux_lm_6_5_ci_lower_data, flux_lm_6_5_ci_upper_data, \
                    meteor_num_data, bin_information = ret

                    # Add computed flux to the output list
                    output_data += [[config.stationID, sol, flux, lower, upper] for (sol, flux, lower, upper) in
                                    zip(sol_data, flux_lm_6_5_data, flux_lm_6_5_ci_lower_data, flux_lm_6_5_ci_upper_data)]
                    ...
                    plt.errorbar(sol_data, flux_lm_6_5_data, color=plot_params['color'], alpha=0.5, capsize=5, zorder=3, linestyle='none', \
        ...
        data = np.genfromtxt(fluxbatch_cml_args.batch_path, delimiter=',', dtype=None, encoding=None, skip_header=1)

        for stationID, sol, flux, lower, upper in data:
            plot_params = plot_info(stationID)

            plt.errorbar(sol, flux, **plot_params, alpha=0.5, capsize=5, zorder=3, linestyle='none',
    ...
    if not fluxbatch_cml_args.csv:
        ...
        with open(data_out_path, 'w') as fout:
            fout.write("#Station, Sol (deg), Flux@+6.5M (met/1000km^2/h), Flux lower bound, Flux upper bound\n")
            for entry in output_data:
                stationID, sol, flux, lower, upper = entry

                fout.write("{:s},{:.8f},{:.3f},{:.3f},{:.3f}\n".format(stationID, sol, flux, lower, upper))
    ...
    if all_bin_information:
        ...
        print(area_time_product)
        sol, flux, flux_lower, flux_upper, num_meteors, ta_prod = \
            combineFixedBinsAndComputeFlux(sol_bins, num_meteors, area_time_product,  ci=ci)
        print(sol, flux, flux_lower, flux_upper, num_meteors, ta_prod, sep='\n')
        plt.errorbar(np.degrees(sol)%360, 
                     flux, yerr=[flux - flux_lower, flux_upper - flux], 
                     label='weighted average flux', c='k', marker='o', linestyle='none')

        if not fluxbatch_cml_args.csv:
            data_out_path = os.path.join(dir_path, f"{fluxbatch_cml_args.output_filename}_2.csv")
            with open(data_out_path, 'w') as fout:
                fout.write("Sol (rad), Flux@+6.5M (met/1000km^2/h), Flux lower bound, Flux upper bound, Area-time product (corrected to +6.5M) (m^3/h), Meteor Count\n")
                for _sol, _flux, _flux_lower, _flux_upper, _at, _nmeteors in \
                    zip(sol, flux, flux_lower, flux_upper, ta_prod, num_meteors):
