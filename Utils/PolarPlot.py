...
def SkyPolarProjection(config_paths, path_to_transform, force_recomputation=False, repeat=False, period=120, print_activity=False, size=500, stack_depth=3, upload=None, annotate=True):
    ...
    while True:
        ...
        min_threshold, max_threshold = np.percentile(intensities, 90), np.percentile(intensities, 100)
        #min_threshold, max_threshold = np.min(target_image_array), np.max(target_image_array)
