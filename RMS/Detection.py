...
def reconstructWindows(path, filename):    
    ...
    for i in range(4, 256/time_slide-1):
        ...
        show(filename + " " + str(i*time_slide) + "-" + str(i*time_slide+time_window_size) + " raw", img)
...
#         show(filename + " " + str(i*time_slide) + "-" + str(i*time_slide+time_window_size) + " morph", img)
        ...
        for i in range(20):
            w = img_cpy.shape[1]
            h = img_cpy.shape[0]
            img = (img_cpy.flatten().astype(np.byte)*255).astype(np.byte)
...
#             print x1, y1, x2, y2
        ...
        cv2.imshow(filename + " " + str(i*time_slide) + "-" + str(i*time_slide+time_window_size) + " KHT", img)
        cv2.moveWindow(filename + " " + str(i*time_slide) + "-" + str(i*time_slide+time_window_size) + " KHT", 500, 500)
        cv2.waitKey(0)
#  *
