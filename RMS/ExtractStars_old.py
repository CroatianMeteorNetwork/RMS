...
def extractStars(ff_dir, ff_name, config=None, max_global_intensity=150, border=10, neighborhood_size=10, 
    ...
    # Just before PSF fitting call
    for idx in range(len(x)):
        x_coord = float(x[idx])
        y_coord = float(y[idx])
        if 869 <= x_coord <= 872 and 369 <= y_coord <= 372:
            print(f"OLD - Star {idx} Before PSF fit: {x_coord:.2f}, {y_coord:.2f}")

    # Fit a PSF to each star
    x2, y2, amplitude, intensity, sigma_y_fitted, sigma_x_fitted = fitPSF(ff, global_mean, x, y, config)

    # After PSF fitting, add this
    for idx in range(len(x2)):
        x_coord = float(x2[idx])
        y_coord = float(y2[idx])
        if 870 <= x_coord <= 872 and 370 <= y_coord <= 372:
            print(f"OLD - Star {idx} After PSF fit: {x_coord:.2f}, {y_coord:.2f}")
...
def fitPSF(ff, avepixel_mean, x2, y2, config):
    ...
    for star in zip(list(y2), list(x2)):
        ...
        if 869 <= x <= 872 and 369 <= y <= 372:
            print(f"OLD - Segment bounds: x_min={x_min}, x_max={x_max}, y_min={y_min}, y_max={y_max}")
        ...
        if 869 <= x <= 872 and 369 <= y <= 372:
            print(f"OLD - Segment shape: {star_seg.shape}")
            print(f"OLD - x_ind shape: {x_ind.shape}, min: {x_ind.min()}, max: {x_ind.max()}")
            print(f"OLD - y_ind shape: {y_ind.shape}, min: {y_ind.min()}, max: {y_ind.max()}")
            print(f"OLD - First few x_ind values: {x_ind[0, 0:3]}, {x_ind[1, 0:3]}")
            print(f"OLD - First few y_ind values: {y_ind[0, 0:3]}, {y_ind[1, 0:3]}")
        ...
        try:
            ...
            opt.curve_fit(twoDGaussian, (y_ind, x_ind, saturation), star_seg.ravel(), \
        ...
        # Add this debugging code
        if 869 <= x <= 872 and 369 <= y <= 372:
            print(f"OLD - Fitted Gaussian center in segment: ({xo:.2f}, {yo:.2f})")
            print(f"OLD - Final position will be: ({x_min + xo:.2f}, {y_min + yo:.2f})")
