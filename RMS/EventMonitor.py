...
class EventMonitor(multiprocessing.Process):
    ...
    def doUpload(self, event, evcon, file_list, keep_files=False, no_upload=False, test_mode=False, write_log=False):
        ...
        output_mp4 = os.path.join(this_event_directory, f"{ev_con.stationID}_{event.dt}_combined.mp4")
        output_mkv = os.path.join(this_event_directory, f"{ev_con.stationID}_{event.dt}_combined.mkv")
