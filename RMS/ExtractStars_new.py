...
def extractStars(img, img_median=None, mask=None, gamma=1.0, max_star_candidates=1000, border=10,
    ...
    for idx in range(len(x_init)):
        x_coord = float(x_init[idx])
        y_coord = float(y_init[idx])
        if 869 <= x_coord <= 872 and 369 <= y_coord <= 372:
            print(f"NEW - Star {idx} Before PSF fit: {x_coord:.2f}, {y_coord:.2f}")
    ...
    for idx in range(len(x_arr)):
        x_coord = float(x_arr[idx])
        y_coord = float(y_arr[idx])
        if 870 <= x_coord <= 872 and 370 <= y_coord <= 372:
            print(f"NEW - Star {idx} After PSF fit: {x_coord:.2f}, {y_coord:.2f}")
...
def extractStarsFF(
    ...
    img_median = np.mean(ff.avepixel)
...
def fitPSF(img, img_median, x_init, y_init, gamma=1.0, segment_radius=4, roundness_threshold=0.5, 
    ...
    for star in zip(list(y_init), list(x_init)):
        ...
        if 869 <= x <= 872 and 369 <= y <= 372:
            print(f"NEW - Segment bounds: x_min={x_min}, x_max={x_max}, y_min={y_min}, y_max={y_max}")
        ...
        # Add this debugging code
        if 869 <= x <= 872 and 369 <= y <= 372:
            print(f"NEW - Fitted Gaussian center in segment: ({xo:.2f}, {yo:.2f})")
            print(f"NEW - Final position will be: ({x_min + xo:.2f}, {y_min + yo:.2f})")
