...
def quotaReport(capt_dir_quota, config, after=False):
    ...
    rep += ("--------------------------------------------\n")
    rep += ("Space used                              \n")
    rep += ("   Total space used for RMS_data : {:07.02f}GB\n".format(usedSpace(config.data_dir)))
    rep += ("            bz2 files space used : {:07.02f}GB\n".format(sizeBz2Files(config)))
    rep += (" archived directories space used : {:07.02f}GB\n".format(sizeArchivedDirs(config)))
    rep += ("              total for archives : {:07.02f}GB\n".format(usedSpace(archived_dir)))
