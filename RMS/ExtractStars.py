...
def extractStarsAuto(img, mask=None, 
    ...
    # The historical intensity-threshold sweep is obsolete: the candidate gate is
    # noise-adaptive, so a single extraction already operates at the depth the image
    ...
    if status is not False:
        ...
        if verbose:
            print("Adaptive gate found {:d} stars".format(len(x_data)))
...
def extractStarsFF(
    ...
    log.debug('extracted {:d} stars from {:s} (noise sigma {:.2f}, beta {:.2f}, gate {:.1f} ADU)'.format(
        len(x_arr), ff_name, extra_info.get('noise_sigma', float('nan')),
        extra_info.get('noise_beta', float('nan')), extra_info.get('gate_adu', float('nan'))))
