...
def extractStars(img, img_median=None, mask=None, gamma=1.0, max_star_candidates=1000, border=10,
    ...
    img_filt = ndimage.filters.convolve(img, weights=np.full((2, 2), 1.0/4))

    # Locate local maxima on the image
    img_max = filters.maximum_filter(img_filt, neighborhood_size)
    maxima = (img_filt == img_max)
    img_min = filters.minimum_filter(img_filt, neighborhood_size)
    ...
    xy = np.array(ndimage.center_of_mass(img_filt, labeled, range(1, num_objects + 1)))
    ...
    # Fit a PSF to each star on the original image
    (
