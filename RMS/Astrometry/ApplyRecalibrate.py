...
def recalibrateIndividualFFsAndApplyAstrometry(dir_path, ftpdetectinfo_path, calstars_list, config, platepar,
    ...
    # Go through all FF files with detections, recalibrate and apply astrometry
    recalibrated_platepars = recalibratePlateparsForFF(prev_platepar, ff_processing_list, calstars, catalog_stars, config)
    ...
    with open(os.path.join(dir_path, config.platepars_recalibrated_name), 'w') as f:
        ...
        # Convert all platepars to a JSON file
        out_str = json.dumps(
            all_pps, default=lambda o: o.__dict__, indent=4, sort_keys=True)
    ...
    if len(recalibrated_platepars) == 0:
        ...
        applyAstrometryFTPdetectinfo(dir_path, os.path.basename(
            ftpdetectinfo_path), None, platepar=platepar)
    ...
    first_dt = np.min([FFfile.filenameToDatetime(ff_name)
                      for ff_name in recalibrated_platepars])
    ...
    if generate_plot:
        ...
        plt.scatter(0, 0, marker='o', edgecolor='k',
                    label='Reference platepar', s=100, c='none', zorder=3)
        ...
        plt.savefig(os.path.join(dir_path, plot_name +
                    '_calibration_variation.png'), dpi=150)
        ...
        plt.savefig(os.path.join(dir_path, plot_name +
                    '_photometry_variation.png'), dpi=150)
    ...
    calib_str = 'Recalibrated with RMS on: ' + \
    ...
    try:
        ...
        print('ERROR! The FTPdetectinfo file could not be backed up: {:s}'.format(
            ftpdetectinfo_path))
...
if __name__ == "__main__":
    ...
    if cml_args.report:
        ...
        Utils.CalibrationReport.generateCalibrationReport(
            config, dir_path, show_graphs=True)
