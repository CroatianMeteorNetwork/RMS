...
def _topic_astrometry(gui):
    ...
    body = (
        ...
        "(CALSTARS) from a coverage-selected subset of the night are matched to the catalog and "
        "their residuals reported by radius, with the corner cells filled whenever the dataset "
        "has stars there. Only the pointing is refit per frame (distortion and scale frozen), so "
        "mount drift over the night is separated from distortion error and reported on its own. "
        "A rising residual toward large radii means the distortion model does not generalize to "
        "the corners; a low corner match fraction means corner stars exist but miss the catalog "
        "by more than the match radius.</p>"
