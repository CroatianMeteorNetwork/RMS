...
def _topic_astrometry(gui):
    ...
    body = (
        ...
        "are kept in full). Two variants are fitted &ndash; with and without compensation of the "
        "measured per-frame pointing drift &ndash; each is validated against the night, and the "
        "best of the three platepars (including the current one) is kept, so a refit never makes "
        "the calibration worse. This typically improves the corners substantially when the "
        "calibration frame had few corner stars; on an already well-generalizing platepar it "
        "simply reports that no improvement was found. Photometry is untouched &ndash; it must "
        "come from a single frame.</p>"
