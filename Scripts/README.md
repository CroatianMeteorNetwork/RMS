...
- `add_Station.sh` creates camera stations under `~/source/Stations/` — interactively, or in bulk from a CSV file (see `stations_template.csv` in the repository root). It also relocates a legacy single-camera setup into that layout. Works on any Linux platform, Raspberry Pi included.
