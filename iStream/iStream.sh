...
function upload_kml_file {
	...
	if [ -f "$KML_FILE_100" ]; then		
		curl --user-agent $AGENT -F"operation=upload" -F"file=@$KML_FILE_100" "$SERVER/$SYSTEM/?station_id=$STATION_ID&type=kml&action=upload&level=$ACTIVE_LEVEL"	
	fi	
	KML_FILE_70="$CAPTURED_DIR_NAME/$STATION_ID-70km.kml"	
	if [ -f "$KML_FILE_70" ]; then		
		curl --user-agent $AGENT -F"operation=upload" -F"file=@$KML_FILE_70" "$SERVER/$SYSTEM/?station_id=$STATION_ID&type=kml&action=upload&level=$ACTIVE_LEVEL"	
	fi	
	KML_FILE_25="$CAPTURED_DIR_NAME/$STATION_ID-25km.kml"	
	if [ -f "$KML_FILE_25" ]; then		
		curl --user-agent $AGENT -F"operation=upload" -F"file=@$KML_FILE_25" "$SERVER/$SYSTEM/?station_id=$STATION_ID&type=kml&action=upload&level=$ACTIVE_LEVEL"	
	fi	
}
